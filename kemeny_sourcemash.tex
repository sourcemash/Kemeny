...
\item Generate a bag-o-words for article $a$, weighting counts for ngrams from the article title and longer phrases (e.g. bigrams, trigrams) more heavily.
...
\item For each keyword candidate $c$:
  \begin{enumerate}
  	...
  	\begin{enumerate}
 		 \item If the article exists but is ambiguous (i.e. has a Wiki disambiguation page), map the keyword $c$ to all related Wikipedia articles linked to from the disambiguation page, denoted $W' = \{w'_{1}, w'_{2}, ...\}$.
  		\item Otherwise, map $c$ to the unambiguous Wikipedia article $w$.
  	\end{enumerate}
	\item Memoize the cross-Wiki links in $w$ (or in all $w' \in W'$) to quickly compute relatedness scores later.
  \end{enumerate}
...
\item For each $c'$ that mapped to an ambiguous set $W'$, sum the relatedness scores between each $w' \in W'$ and all articles $w \in A$ from Step 4. Append the $w'$ with the greatest summed relatedness score to the list of assigned articles $A$.
